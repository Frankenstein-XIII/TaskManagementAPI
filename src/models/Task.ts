...
export abstract class BaseTask implements ITask {
  ...
  // make tTtype abstract so the subclIUserass must define it.
  public abstract tType: "bug" | "feature";
  public ownerId: string = "";
  public dueDate: Date;

  constructor(public title: string, dueDateString: string) {
  ...
  isoverDue(): boolean{
      return this.status !=='closed' && new Date() > this.dueDate;
  }
...
export class BugTask extends BaseTask{
      ...
      constructor(title: string, public severity: 'low'|'high', dueDate: string){
            super(title, dueDate);
      }
...
export class FeatureTask extends BaseTask{
      ...
      constructor(title: string, public impactScore: number, dueDate: string){
            super(title, dueDate);
      }
